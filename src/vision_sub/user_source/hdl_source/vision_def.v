...
// 极性探测失败后的自动重试周期：2s @50MHz
//   若两种极性都读不到 0x1324（cam_ok=0），每 2s 自动重走一次完整探测。
//   意义：现场排障时"改接线 → 补上拉 → 复位重试"是最慢的一环，自动重试
//   把这一步变成"改完等 2 秒"。探测成功（cam_ok=1）后不再重试。
//   探测量很小：一次失败的重试只发两条读事务（约 1.2ms / 2s）。
`define PROBE_RETRY_CNT  32'd100000000
