...
module app_wrrd 
	(   
		...
		output [11:0] wr_fifo_usedw   // additive: u_wr_fifo_sdr_data occupancy -> scaler backpressure
		// output	reg		Check_ok	
	);
reg [18:0]w_addr_cnt;
// reg	[2:0]	judge_cnt;
...
wire  [11:0] wrusedw_full;// write-FIFO occupancy (u_wr_fifo_sdr_data is 4096 deep)
...
fifo_sdr_data_2 u_wr_fifo_sdr_data(
   ...
   .	wrusedw	  	    	(wrusedw_full	)	,  	//	stored data number in fifo
   .	rdusedw 	    	(rdusedw 	)//available data number for read //会持续两个快的时钟周期
 
);

assign wr_fifo_usedw = wrusedw_full;   // additive: raw occupancy; parent saturates to scaler's 9-bit port
